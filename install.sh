...
# Обновляем временные метки кэш-файлов, чтобы они считались свежими при первом запуске
touch /etc/storage/ipset_domains.cache /etc/storage/ipset_ips.cache /etc/storage/learned_ips.cache 2>/dev/null
